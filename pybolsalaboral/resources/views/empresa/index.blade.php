    ...
    <div class="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
        <div class="flex justify-between">
            ...
            <div class="order-last">
                <a href="{{ url('empresa/') }}" class="text-teal-900 font-bold py-1 px-2 shadow-md rounded-full">
                        ...
                        <div class="form-row flex">
                            ...
                            <div class="col-auto my-1 ml-32" class="lg:ml-40 ml-10 space-x-8" >
                                <button
                                    ...
                                    <i class="fa-solid fa-file"></i> <a href="{{ url('empresas/pdf') }}">PDF</a>
                                </button>
                            </div>
                            ...
                            <div class="col-auto my-1 ml-5" class="lg:ml-40 ml-10 space-x-8">
                                <button wire:click="create()"
                                    class="bg-indigo-600 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer">
                                    <i class="fa-solid fa-plus"></i> <a href="{{ url('empresa/create') }}">Registrar
                                        nuevo empresa</a>
                                </button>
                            </div>
                    ...
                    <table class="w-full divide-y divide-gray-200 table-auto">
                        <thead class="bg-indigo-500 text-white">
                            <tr class="text-left text-xs font-bold  uppercase">
                                ...
                                <td scope="col" class="px-6 py-3">Numero de trabajadores</td>
                            ...
                            @else
                                @foreach ($empresas as $empresa)
                                    <tr class="text-sm font-medium text-gray-900">
                                        ...
                                        <td class="px-6 py-4">{{ $empresa->ep_rubro }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_anios_actividad }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_num_trabajadores }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_num_cedes }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_sitio_web }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_nombre }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_direccion }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_correo }}</td>
                                        <td class="px-6 py-4">{{ $empresa->ep_celular }}</td>
                                        <td class="px-6 py-4">
